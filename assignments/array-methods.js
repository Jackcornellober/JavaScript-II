...
let capMap = runners.map(runner => runner.first_name.toUpperCase())
console.log(capMap); 
...
// Problem 1
    //The event director wants to know which companies donated more than $200. Make it so.

let philanthropists = runners.filter(runner => runner.donation > 199).map(runner => runner.company_name)
console.log(philanthropists)

// Problem 2
    //The event director wants a list of all of the emails separated by commas in one string so that he can easily send them all an email. Do it up.

    let emails = runners.reduce((email, runner) => email + runner.email + ", ", "")
    console.log(emails)

// Problem 3
    //The event director has gone home for the night. It's just you in the office now. Shave $10 from each donation and keep it all for yourself.

let ibizaVacationFund = 0
let totallyLegitRunnersArrayNotTamperedWithAtAll = runners.map(runner => {runner.donation = runner.donation -10; ibizaVacationFund = ibizaVacationFund + 10; return runner;})
console.log(totallyLegitRunnersArrayNotTamperedWithAtAll)
console.log("Ibiza Vacation Fund: $" + ibizaVacationFund)
